    ...
    getMonitorItemProperties: function (item, propertyCodes) {
      ...
      propertyCodes.forEach(code => {
        switch (code) {
          ...
          // Исполнитель
          case 'contractor':
            // let hasContractor = Boolean(typeof item.contractor !== 'undefined' && item.contractor != null)
            // if (item.contractor_id)
            // let contractor = this.$store.getters['orgStructure/postInfo']( item.contractor_id )
            let contractor = {
              avatar: true,
              // image: item.customer.image ? item.customer.image : '',
              avatarIcon: 'face',
              title: item.contractor_id 
                ? 'Тестовый пользователь'
                : 'Не назначен',
              subTitle: item.contractor_id 
            ...
            // console.log("getMonitorItemProperties(), contractor:");
            // console.log(contractor);
            properties.push(contractor)
            break;
          ...
          case 'time_limit':
            properties.push({
              ...
              title: item.time_limit
                ? (this.getTimeValue( item.time_limit ) + " " + this.getTimeTitle( item.time_limit ) 
                  + " (до " + this.getHumanDateFromSec( item.started_at + item.time_limit, "D MMMM YYYY, H:mm" ) + ")")
                : 'Не установлен',
