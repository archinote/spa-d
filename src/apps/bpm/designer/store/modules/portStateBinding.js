import apiPortStateBinding from '../../api/portStateBinding'
...
const mutations = {
  ...
  list: (state, { parentId, list }) => {
    if (parentId && list) {
      ...
      // console.log("portStateBinding Store, 'list' mutation, state.items:")
...
const actions = {
  ...
  list ({ commit, getters }, { parentId }) {
    return new Promise( (resolve, reject) => {
      ...
      if (_items && _items.length) {
        ...
        apiPortStateBinding.list( parentId ).then(list => {
          // console.log("portStateBinding Store, 'list' action, API response:")
          // console.log(list)
  ...
  item ({ dispatch, commit, getters }, { id, parentId }) {
    return new Promise( (resolve, reject) => {
      ...
      if (_item) {
        ...
        apiPortStateBinding.item(parentId, id).then(_item => {
          console.log("portStateBinding Store, 'item' action, API response:")
          console.log(_item)
  ...
  create ({ commit, getters }, { parentId, item }) {
    return new Promise( (resolve, reject) => {
      ...
      apiPortStateBinding.create( parentId, item ).then(item => {
        console.log("portStateBinding Store, 'create' action, API response:")
        console.log(item)
  ...
  update ({ commit, getters }, { parentId, data }) {
    return new Promise( (resolve, reject) => {
      // Try to send data on the server.
      apiPortStateBinding.update( parentId, data ).then(response => {
        // console.log("portStateBinding Store, 'update' action, API response:")
  ...
  delete ({ commit, getters }, { parentId, id }) {
    return new Promise( (resolve, reject) => {
      // Try to send data on the server.
      apiPortStateBinding.delete( parentId, id ).then(response => {
        console.log("portStateBinding Store, 'delete' action, API response:")
        console.log(response)
