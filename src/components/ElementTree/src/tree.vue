<template>
  <div class="d-tree" :class="{
      'd-tree--highlight-current': highlightCurrent,
      'is-dragging': !!dragState.draggingNode,
      'is-drop-not-allow': !dragState.allowDrop,
      'is-drop-inner': dragState.dropType === 'inner'
    }" role="tree">
    <d-tree-node v-for="child in root.childNodes" :node="child" :props="props" :render-after-expand="renderAfterExpand" :key="getNodeKey(child)" :render-content="renderContent" @node-expand="handleNodeExpand"></d-tree-node>
    <template v-if="isEmpty">
      <div class="d-tree__empty-block">
        <span v-if="!loading" class="d-tree__empty-text">{{ emptyText }}</span>
        <template v-else>
          <v-progress-circular :size="50" indeterminate color="teal"></v-progress-circular>
        </template>
      </div>
    </template>
    <div v-show="dragState.showDropIndicator" class="d-tree__drop-indicator" ref="dropIndicator"></div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    emptyText: {
      type: String,
      default() {
        return t('Нет ни одного эелемента')
      }
    },
    renderAfterExpand: {
      type: Boolean,
  ...
  created() {
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      const targetPosition = dropNode.$el.querySelector('.d-tree-node__expand-icon').getBoundingClientRect()
